			...
			for _, padding := range paddings {
				t.Run(fmt.Sprintf("Padding_%d", padding), func(t *testing.T) {
					...
					for connIdx, conn := range tt.conns {
						...
						var source, target *diagram.Node
						...
						if source.ID == target.ID {
							sourcePoint = diagram.Point{X: source.X + source.Width/2, Y: source.Y + source.Height}
							targetPoint = diagram.Point{X: source.X + source.Width + 2, Y: source.Y + source.Height/2}
						}
...
func createObstaclesFunctionWithPadding(nodes []diagram.Node, sourceID, targetID int, padding int) func(diagram.Point) bool {
...
func TestObstacleBoundaryPrecision(t *testing.T) {
	...
	testCases := []struct {
		...
		{diagram.Point{X: 9, Y: 10}, 0, false, "Just left of node"},
		{diagram.Point{X: 10, Y: 10}, 0, true, "Top-left corner"},
		{diagram.Point{X: 14, Y: 10}, 0, true, "Top-right corner"},
		{diagram.Point{X: 15, Y: 10}, 0, false, "Just right of node"},
		{diagram.Point{X: 10, Y: 9}, 0, false, "Just above node"},
		{diagram.Point{X: 10, Y: 12}, 0, true, "Bottom-left corner"},
		{diagram.Point{X: 10, Y: 13}, 0, false, "Just below node"},
		
		// Test with padding=1
		{diagram.Point{X: 9, Y: 10}, 1, true, "In padding zone left"},
		{diagram.Point{X: 15, Y: 10}, 1, true, "In padding zone right"},
		{diagram.Point{X: 10, Y: 9}, 1, true, "In padding zone top"},
		{diagram.Point{X: 10, Y: 13}, 1, true, "In padding zone bottom"},
		{diagram.Point{X: 8, Y: 10}, 1, false, "Outside padding zone"},
		
		// Test with padding=2
		{diagram.Point{X: 8, Y: 10}, 2, true, "In padding=2 zone"},
		{diagram.Point{X: 16, Y: 10}, 2, true, "In padding=2 zone right"},
		{diagram.Point{X: 7, Y: 10}, 2, false, "Outside padding=2 zone"},
	}
	
	for _, tc := range testCases {
		obstacles := createObstaclesFunctionWithPadding([]diagram.Node{node}, -1, -1, tc.padding)
