...
func NewSimpleRouter(pathFinder diagram.PathFinder, obstacleManager ObstacleManager) *SimpleRouter {
	return &SimpleRouter{
...
func (sr *SimpleRouter) RouteConnection(conn diagram.Connection, nodes []diagram.Node) (diagram.Path, error) {
	...
	if err != nil {
		return diagram.Path{}, fmt.Errorf("failed to find path: %w", err)
	}
...
func (sr *SimpleRouter) createCenterToCenterObstacles(nodes []diagram.Node, sourceID, targetID int) func(diagram.Point) bool {
...
func (sr *SimpleRouter) truncatePath(path diagram.Path, sourceNode, targetNode *diagram.Node) diagram.Path {
	if len(path.Points) < 2 {
		return path
	}
	
	points := path.Points
	truncatedPoints := make([]diagram.Point, 0)
	
	// Find where path exits source node
	sourceExitPoint := diagram.Point{}
	...
	targetEntryPoint := diagram.Point{}
...
func findNodeEdgeIntersection(from, to diagram.Point, node *diagram.Node, isExit bool) diagram.Point {
	...
	if dx != 0 {
		...
		if (from.X < left && to.X >= left) || (from.X >= left && to.X < left) {
			...
			if y >= top && y <= bottom {
				return diagram.Point{X: left, Y: y}
			}
		}
		...
		if (from.X <= right && to.X > right) || (from.X > right && to.X <= right) {
			...
			y := from.Y + int(t*float64(dy))
			if y >= top && y <= bottom {
				return diagram.Point{X: right, Y: y}
			}
		}
	}
	...
	if dy != 0 {
		...
		if (from.Y < top && to.Y >= top) || (from.Y >= top && to.Y < top) {
			...
			if x >= left && x <= right {
				return diagram.Point{X: x, Y: top}
			}
		}
		...
		if (from.Y <= bottom && to.Y > bottom) || (from.Y > bottom && to.Y <= bottom) {
			...
			x := from.X + int(t*float64(dx))
			if x >= left && x <= right {
				return diagram.Point{X: x, Y: bottom}
			}
		}
	}
	...
	if geometry.Abs(to.X - centerX) > geometry.Abs(to.Y - centerY) {
		...
		if to.X > centerX {
			...
			return diagram.Point{X: left, Y: centerY}
		}
	} else {
		// Vertical edge
		if to.Y > centerY {
			return diagram.Point{X: centerX, Y: bottom}
		} else {
			return diagram.Point{X: centerX, Y: top}
		}
	}
}

// isInsideNode checks if a point is inside a node
func isInsideNode(p diagram.Point, node *diagram.Node) bool {
...
func getNodeEdgePoint(fromNode, toNode *diagram.Node) diagram.Point {
	// Calculate centers
