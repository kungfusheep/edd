...
func NewTwoPhaseRouter(pathFinder diagram.PathFinder, obstacleManager ObstacleManager) *TwoPhaseRouter {
...
func (r *TwoPhaseRouter) SetPortManager(pm PortManager) {
	r.portManager = pm
...
func (r *TwoPhaseRouter) RouteConnectionWithPorts(conn diagram.Connection, nodes []diagram.Node) (diagram.Path, error) {
	...
	if err != nil {
		return diagram.Path{}, fmt.Errorf("failed to find rough path: %w", err)
	}
	...
	var sourcePreferredPoint diagram.Point
	if sourceEdge == East || sourceEdge == West {
		// For vertical edges, align port Y with target center Y
		if sourceEdge == East {
			sourcePreferredPoint = diagram.Point{X: sourceNode.X + sourceNode.Width, Y: targetCenter.Y}
		} else {
			sourcePreferredPoint = diagram.Point{X: sourceNode.X - 1, Y: targetCenter.Y}
		}
	} else {
	...
	var targetPreferredPoint diagram.Point
	if targetEdge == East || targetEdge == West {
		// For vertical edges, align port Y with source center Y
		if targetEdge == East {
			targetPreferredPoint = diagram.Point{X: targetNode.X + targetNode.Width, Y: sourceCenter.Y}
		} else {
			targetPreferredPoint = diagram.Point{X: targetNode.X - 1, Y: sourceCenter.Y}
		}
	} else {
	...
	if err != nil {
		return diagram.Path{}, fmt.Errorf("failed to reserve source port: %w", err)
	}
	...
	if err != nil {
		...
		return diagram.Path{}, fmt.Errorf("failed to reserve target port: %w", err)
	}
	...
	if sourceWaypoint == sourcePort.Point && targetWaypoint == targetPort.Point {
		...
		if err != nil {
			...
			return diagram.Path{}, fmt.Errorf("failed to find direct path: %w", err)
		}
	} else {
		...
		for i, seg := range segments {
			...
			if err != nil {
				r.portManager.ReleasePort(sourcePort)
				r.portManager.ReleasePort(targetPort)
				return diagram.Path{}, fmt.Errorf("failed to find path %s: %w", seg.name, err)
			}
	...
	finalPath = OptimizePath(finalPath, obstacleFunc)
...
func (r *TwoPhaseRouter) findRoughPath(conn diagram.Connection, sourceNode, targetNode *diagram.Node, nodes []diagram.Node) (diagram.Path, EdgeSide, EdgeSide, error) {
	...
	if err != nil {
		return diagram.Path{}, 0, 0, fmt.Errorf("failed to find path from (%d,%d) to (%d,%d): %w", 
...
func getEdgeCenterForPort(node *diagram.Node, edge EdgeSide) diagram.Point {
	switch edge {
	case North:
		return diagram.Point{X: node.X + node.Width/2, Y: node.Y - 1}
	case South:
		return diagram.Point{X: node.X + node.Width/2, Y: node.Y + node.Height}
	case East:
		return diagram.Point{X: node.X + node.Width, Y: node.Y + node.Height/2}
	case West:
		return diagram.Point{X: node.X - 1, Y: node.Y + node.Height/2}
	}
	return diagram.Point{} // Should not reach here
}

// createRoughPathObstacles creates obstacles for rough path that allows centers
func (r *TwoPhaseRouter) createRoughPathObstacles(nodes []diagram.Node, conn diagram.Connection) func(diagram.Point) bool {
...
func (r *TwoPhaseRouter) createPerpendicularWaypoint(port Port, node *diagram.Node, maxDistance int, obstacleFunc func(diagram.Point) bool) diagram.Point {
	// Try different distances to find a clear waypoint
	for distance := maxDistance; distance > 0; distance-- {
		var waypoint diagram.Point
		switch port.Edge {
		case North:
			waypoint = diagram.Point{X: port.Point.X, Y: port.Point.Y - distance}
		case South:
			waypoint = diagram.Point{X: port.Point.X, Y: port.Point.Y + distance}
		case East:
			waypoint = diagram.Point{X: port.Point.X + distance, Y: port.Point.Y}
		case West:
			waypoint = diagram.Point{X: port.Point.X - distance, Y: port.Point.Y}
		default:
			return port.Point
...
func HandleSelfLoops(conn diagram.Connection, node *diagram.Node) diagram.Path {
	...
	if aspectRatio > 1.5 {
		...
		return diagram.Path{Points: points}
	} else {
...
func calculateAngle(from, to diagram.Point) float64 {
...
func selectEdgeByAngle(angle float64) EdgeSide {
...
func (r *TwoPhaseRouter) calculatePreferredPortPosition(node *diagram.Node, edge EdgeSide, angle float64) diagram.Point {
	// Normalize angle
	for angle < 0 {
		angle += 360
	}
	...
	case North, South:
		...
		if edge == North {
			return diagram.Point{X: idealX, Y: node.Y - 1}
		} else {
			return diagram.Point{X: idealX, Y: node.Y + node.Height}
		}
	...
	case East, West:
		...
		if edge == East {
			return diagram.Point{X: node.X + node.Width, Y: idealY}
		} else {
			return diagram.Point{X: node.X - 1, Y: idealY}
		}
	}
