...
func TestNodeRendererBasicStyles(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Check corners
			if canvas.Get(core.Point{X: 2, Y: 2}) != tt.wantChars.topLeft {
				t.Errorf("Top-left corner: got %c, want %c", 
					canvas.Get(core.Point{X: 2, Y: 2}), tt.wantChars.topLeft)
			}
			if canvas.Get(core.Point{X: 11, Y: 2}) != tt.wantChars.topRight {
				t.Errorf("Top-right corner: got %c, want %c", 
					canvas.Get(core.Point{X: 11, Y: 2}), tt.wantChars.topRight)
			}
			if canvas.Get(core.Point{X: 2, Y: 5}) != tt.wantChars.bottomLeft {
				t.Errorf("Bottom-left corner: got %c, want %c", 
					canvas.Get(core.Point{X: 2, Y: 5}), tt.wantChars.bottomLeft)
			}
			if canvas.Get(core.Point{X: 11, Y: 5}) != tt.wantChars.bottomRight {
				t.Errorf("Bottom-right corner: got %c, want %c", 
					canvas.Get(core.Point{X: 11, Y: 5}), tt.wantChars.bottomRight)
			}
		})
	}
}
...
func TestNodeRendererFallback(t *testing.T) {
	...
	if canvas.Get(core.Point{X: 2, Y: 2}) != '╭' {
		t.Errorf("Should fall back to rounded style, got %c", canvas.Get(core.Point{X: 2, Y: 2}))
	}
}
...
func TestNodeRendererASCIIFallback(t *testing.T) {
	...
	if canvas.Get(core.Point{X: 2, Y: 2}) != '+' {
		t.Errorf("Should use ASCII style for ASCII terminal, got %c", canvas.Get(core.Point{X: 2, Y: 2}))
	}
}
...
func TestNodeRendererColors(t *testing.T) {
	...
	if canvas.Get(core.Point{X: 2, Y: 2}) != '╔' {
		t.Errorf("Expected double-line top-left corner, got %c", canvas.Get(core.Point{X: 2, Y: 2}))
	}
