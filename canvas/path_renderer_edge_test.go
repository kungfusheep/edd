  ...
  •  
     ...
     `,
		},
		{
			...
			caps: TerminalCapabilities{UnicodeLevel: UnicodeFull},
			path: core.Path{}, // We'll render two paths
			hasArrow: false,
			width:    7,
			height:   5,
		},
		...
		},
		{
			...
			caps: TerminalCapabilities{UnicodeLevel: UnicodeFull},
			path: core.Path{
				Points: []core.Point{{X: 2, Y: 2}, {X: 2, Y: 2}},
			},
			hasArrow: false,
			width:    5,
			height:   5,
     ...
     `,
		},
		{
			...
			caps: TerminalCapabilities{UnicodeLevel: UnicodeFull},
			path: core.Path{
				Points: []core.Point{{X: 4, Y: 2}, {X: 1, Y: 2}},
			},
			hasArrow: true,
			width:    6,
			height:   4,
  ...
  ◀── 
      `,
		},
		{
			...
			caps: TerminalCapabilities{UnicodeLevel: UnicodeFull},
			path: core.Path{
				Points: []core.Point{{X: 2, Y: 3}, {X: 2, Y: 1}},
			},
			hasArrow: true,
			width:    5,
			height:   5,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.name == "crossing paths horizontal/vertical" {
				...
				path1 := core.Path{Points: []core.Point{{X: 1, Y: 2}, {X: 5, Y: 2}}}
				renderer.RenderPath(c, path1, false)
				
				// Render vertical line crossing it
				path2 := core.Path{Points: []core.Point{{X: 3, Y: 0}, {X: 3, Y: 4}}}
   ...
   │   
       `
			...
			if tt.name == "path with junction at corner" {
				...
				path1 := core.Path{Points: []core.Point{{X: 1, Y: 1}, {X: 3, Y: 1}, {X: 3, Y: 3}}}
				renderer.RenderPath(c, path1, false)
				
				// Render line that intersects at the corner
				path2 := core.Path{Points: []core.Point{{X: 3, Y: 0}, {X: 3, Y: 2}}}
				renderer.RenderPath(c, path2, false)
