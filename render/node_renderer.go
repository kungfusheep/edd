...
func (r *NodeRenderer) RenderNodeWithHints(canvas Canvas, node diagram.Node, hints map[string]string) error {
...
func (r *NodeRenderer) RenderShadowOnly(canvas Canvas, node diagram.Node) {
	if node.Hints != nil {
...
func (r *NodeRenderer) drawShadow(canvas Canvas, node diagram.Node, direction string, density string) {
	...
	if direction == "southeast" || direction != "" {
		...
		for y := node.Y + 1; y <= node.Y + node.Height - 1; y++ {
			r.setChar(canvas, diagram.Point{X: node.X + node.Width, Y: y}, shadowChar, "")
		}
		// Bottom edge shadow (full width + corner)
		for x := node.X + 1; x <= node.X + node.Width; x++ {
			r.setChar(canvas, diagram.Point{X: x, Y: node.Y + node.Height}, shadowChar, "")
		}
	}
}

// drawBox draws the box border for a node
func (r *NodeRenderer) drawBox(canvas Canvas, node diagram.Node, style NodeStyle, color string) error {
	// Top border
	r.setChar(canvas, diagram.Point{X: node.X, Y: node.Y}, style.TopLeft, color)
	for x := node.X + 1; x < node.X + node.Width - 1; x++ {
		r.setChar(canvas, diagram.Point{X: x, Y: node.Y}, style.Horizontal, color)
	}
	r.setChar(canvas, diagram.Point{X: node.X + node.Width - 1, Y: node.Y}, style.TopRight, color)
	
	// Side borders
	for y := node.Y + 1; y < node.Y + node.Height - 1; y++ {
		r.setChar(canvas, diagram.Point{X: node.X, Y: y}, style.Vertical, color)
		r.setChar(canvas, diagram.Point{X: node.X + node.Width - 1, Y: y}, style.Vertical, color)
	}
	
	// Bottom border
	r.setChar(canvas, diagram.Point{X: node.X, Y: node.Y + node.Height - 1}, style.BottomLeft, color)
	for x := node.X + 1; x < node.X + node.Width - 1; x++ {
		r.setChar(canvas, diagram.Point{X: x, Y: node.Y + node.Height - 1}, style.Horizontal, color)
	}
	r.setChar(canvas, diagram.Point{X: node.X + node.Width - 1, Y: node.Y + node.Height - 1}, style.BottomRight, color)
	
	return nil
}

// drawText draws the text content inside a node
func (r *NodeRenderer) drawText(canvas Canvas, node diagram.Node, hints map[string]string) error {
	...
	for i, line := range node.Text {
		...
		if isCenter {
			...
			r.setCharWithStyle(canvas, diagram.Point{X: x, Y: y}, ' ', textColor, isBold, isItalic)
			x++
		}
		...
		if !isCenter {
			...
			if textEnd < node.X+node.Width-1 {
				r.setCharWithStyle(canvas, diagram.Point{X: textEnd, Y: y}, ' ', textColor, isBold, isItalic)
			}
		}
	}
...
func (r *NodeRenderer) setChar(canvas Canvas, p diagram.Point, char rune, color string) {
	if color != "" {
...
func (r *NodeRenderer) setCharWithStyle(canvas Canvas, p diagram.Point, char rune, color string, bold bool, italic bool) {
	// Build style string
	style := ""
	if bold && italic {
		style = "bold+italic"
