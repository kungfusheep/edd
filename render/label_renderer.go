...
func (lr *LabelRenderer) RenderLabel(c Canvas, path diagram.Path, label string, position LabelPosition) {
...
func (lr *LabelRenderer) findBestSegmentForLabel(path diagram.Path, label string, position LabelPosition) *Segment {
	...
	for _, seg := range segments {
		var segLen int
		if seg.IsHorizontal {
			segLen = geometry.Abs(seg.End.X - seg.Start.X)
		} else if seg.IsVertical {
			segLen = geometry.Abs(seg.End.Y - seg.Start.Y)
		} else {
...
func (lr *LabelRenderer) combineConsecutiveSegments(path diagram.Path) []Segment {
	if len(path.Points) < 2 {
		return nil
	}
...
func getSegmentDirection(from, to diagram.Point) string {
	dx := to.X - from.X
	dy := to.Y - from.Y
...
func (lr *LabelRenderer) findAnySegmentForLabel(path diagram.Path, label string, position LabelPosition) *Segment {
	if len(path.Points) < 2 {
		return nil
	}
...
func (lr *LabelRenderer) renderHorizontalInlineLabel(c Canvas, segment *Segment, label string) {
	labelLen := len(label)
	segmentLen := geometry.Abs(segment.End.X - segment.Start.X)
...
func (lr *LabelRenderer) renderVerticalInlineLabel(c Canvas, segment *Segment, label string) {
	...
	labelLen := len(label)
	segmentLen := geometry.Abs(segment.End.Y - segment.Start.Y)
