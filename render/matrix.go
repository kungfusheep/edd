...
//	var mu sync.Mutex
//	mu.Lock()
//	render.DrawBox(10, 10, 20, 20, DefaultBoxStyle)
//	mu.Unlock()
//
...
func (c *MatrixCanvas) Get(p diagram.Point) rune {
...
func (c *MatrixCanvas) Set(p diagram.Point, char rune) error {
	if p.X < 0 || p.X >= c.width || p.Y < 0 || p.Y >= c.height {
...
func (c *MatrixCanvas) DrawBox(x, y, width, height int, style BoxStyle) error {
	...
	// Top line
	c.Set(diagram.Point{x, y}, style.TopLeft)
	for i := 1; i < width-1; i++ {
		c.Set(diagram.Point{x + i, y}, style.Horizontal)
	}
	c.Set(diagram.Point{x + width - 1, y}, style.TopRight)
	
	// Vertical lines
	for i := 1; i < height-1; i++ {
		c.Set(diagram.Point{x, y + i}, style.Vertical)
		c.Set(diagram.Point{x + width - 1, y + i}, style.Vertical)
	}
	
	// Bottom line
	c.Set(diagram.Point{x, y + height - 1}, style.BottomLeft)
	for i := 1; i < width-1; i++ {
		c.Set(diagram.Point{x + i, y + height - 1}, style.Horizontal)
	}
	c.Set(diagram.Point{x + width - 1, y + height - 1}, style.BottomRight)
...
func (c *MatrixCanvas) DrawLine(p1, p2 diagram.Point, char rune) error {
...
func (c *MatrixCanvas) DrawSmartPath(points []diagram.Point) error {
	if len(points) < 2 {
...
func getDirection(p1, p2 diagram.Point) rune {
	if p2.X > p1.X {
		return 'E'
	} else if p2.X < p1.X {
		return 'W'
...
func (c *MatrixCanvas) selectCorner(prev, curr, next diagram.Point) rune {
