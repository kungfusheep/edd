...
func (dv *DebugVisualizer) VisualizeObstacles(bounds diagram.Bounds, zones []pathfinding.ObstacleZone, ports []pathfinding.Port) string {
	// Create a 2D grid
...
func (dv *DebugVisualizer) markPort(grid [][]rune, port pathfinding.Port, bounds diagram.Bounds, char rune) {
	x := port.Point.X
	y := port.Point.Y
...
func ExportObstacleData(zones []pathfinding.ObstacleZone, ports []pathfinding.Port) string {
	var result strings.Builder
	...
	for i, port := range ports {
		...
		result.WriteString(fmt.Sprintf("Port %d: node=%d, edge=%s, pos=%d, %s\n",
			i, port.NodeID, pathfinding.EdgeName(port.Edge), port.Position, status))
	}
