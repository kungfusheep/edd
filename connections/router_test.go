...
func TestRouter_RouteConnection(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			conn:    core.Connection{From: 1, To: 2},
			wantErr: false,
			minLen:  2,
		},
		{
			name:    "diagonal connection",
			conn:    core.Connection{From: 1, To: 3},
			wantErr: false,
			minLen:  2,
		},
		{
			name:    "reverse connection",
			conn:    core.Connection{From: 2, To: 1},
			wantErr: false,
			minLen:  2,
		},
		{
			name:    "non-existent source",
			conn:    core.Connection{From: 99, To: 2},
			wantErr: true,
		},
		{
			name:    "non-existent target",
			conn:    core.Connection{From: 1, To: 99},
			wantErr: true,
		},
	}
...
func TestGetConnectionPoint(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			fromNode: core.Node{X: 10, Y: 10, Width: 10, Height: 10},
			toNode:   core.Node{X: 30, Y: 10, Width: 10, Height: 10},
			wantSide: "right",
		},
		{
			name:     "horizontal left",
			fromNode: core.Node{X: 30, Y: 10, Width: 10, Height: 10},
			toNode:   core.Node{X: 10, Y: 10, Width: 10, Height: 10},
			wantSide: "left",
		},
		{
			name:     "vertical down",
			fromNode: core.Node{X: 10, Y: 10, Width: 10, Height: 10},
			toNode:   core.Node{X: 10, Y: 30, Width: 10, Height: 10},
			wantSide: "bottom",
		},
		{
			name:     "vertical up",
			fromNode: core.Node{X: 10, Y: 30, Width: 10, Height: 10},
			toNode:   core.Node{X: 10, Y: 10, Width: 10, Height: 10},
			wantSide: "top",
		},
		{
			...
			fromNode: core.Node{X: 10, Y: 10, Width: 10, Height: 10},
			toNode:   core.Node{X: 25, Y: 20, Width: 10, Height: 10},
...
func findNode(nodes []core.Node, id int) *core.Node {
	for i := range nodes {
		if nodes[i].ID == id {
			return &nodes[i]
		}
...
func isAdjacentToNode(point core.Point, node *core.Node) bool {
	if node == nil {
		return false
	}
