...
func TestArrowConfig(t *testing.T) {
	...
	conn1 := core.Connection{From: 1, To: 2}
	...
	conn2 := core.Connection{From: 2, To: 3}
...
func TestArrowConfig_ShouldDrawArrow(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			conn := core.Connection{From: tt.from, To: tt.to}
...
func TestApplyArrowConfig(t *testing.T) {
	...
	paths := map[int]core.Path{
		0: {Points: []core.Point{{0, 0}, {10, 0}}},
		1: {Points: []core.Point{{10, 0}, {10, 10}}},
		2: {Points: []core.Point{{10, 10}, {0, 0}}},
	}
