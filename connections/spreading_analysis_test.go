...
func TestSpreadingAnalysis(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Create connections
			connections := make([]core.Connection, tt.connCount)
			for i := 0; i < tt.connCount; i++ {
				connections[i] = core.Connection{From: 1, To: 2}
			}
...
func analyzeSpreadingQuality(t *testing.T, name string, nodes []core.Node, paths map[int]core.Path, expectedCount int) {
	...
	startPoints := make([]core.Point, 0, len(paths))
	endPoints := make([]core.Point, 0, len(paths))
...
func TestBidirectionalSpacing(t *testing.T) {
	...
	forwardPaths := make([]core.Path, 0)
	backwardPaths := make([]core.Path, 0)
...
func pathsCross(path1, path2 core.Path) bool {
